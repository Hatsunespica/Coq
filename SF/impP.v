...
Definition isWhite (c:ascii):bool :=
  ...
  orb (orb (beq_nat n 32) (beq_nat n 9))
      (orb (beq_nat n 10) (beq_nat n 13)).
...
Fixpoint parsePrimaryExp (steps: nat) (xs : list token)
  ...
  | S steps' =>
    TRY ' (i, rest) <- parseIdentifier xs ;; SomeE (AId i, rest)
        OR
...
Fixpoint parseAtomicExp (steps: nat) (xs : list token):=
  ...
  | S steps' =>
    TRY ' (u,rest) <- expect "true" xs ;;
        ...
        SomeE (BFalse, rest)
        OR
        TRY ' (e,rest) <- firstExpect "~" (parseAtomicExp steps') xs ;;
        SomeE (BNot e,rest)
        OR
...
Fixpoint parseSimpleCommand (steps :nat) (xs : list token) :=
  ...
  | S steps' =>
    TRY ' (u,rest) <- expect "SKIP" xs ;;
        SomeE (SKIP%imp, rest)
        OR
        TRY ' (e, rest) <- firstExpect "TEST" (parseBExp steps') xs ;;
...
with parseSequencedCommand (steps:nat) (xs : list token) :=
       match steps with
       | 0 => NoneE "Too many recursive calls"
       | S steps' => ' (c, rest) <- parseSimpleCommand steps' xs ;;
